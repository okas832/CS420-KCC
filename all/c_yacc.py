...
def p_defv_1(p):
    ...
    p[0] = VDEF(p[1], p[2], Ln(p.linespan(0)))
...
def p_id_adv_1(p):
    """id_adv : ID"""
    p[0] = VDEFID(p[1], 0, None, Ln(p.linespan(0)))
...
def p_stmt_1(p):
    """stmt : SEMICOL"""
    p[0] = EMPTY_STMT(Ln(p.linespan(0)))
...
def p_stmt_4(p):
    ...
    p[0] = WHILE(p[3], p[5], Ln(p.linespan(0)))


# FOR
def p_stmt_5(p):
    """stmt : FOR LPAREN expr_many SEMICOL expr_many SEMICOL expr_many RPAREN stmt"""
    p[0] = FOR(p[3], p[5], p[7], p[9], Ln(p.linespan(0)))


# COND, IF
def p_stmt_6(p):
    """stmt : IF LPAREN expr_many RPAREN stmt %prec PREC_IF"""
    p[0] = IFELSE(p[3], p[5], None, Ln(p.linespan(0)))


# COND, IF - ELSE
def p_stmt_7(p):
    """stmt : IF LPAREN expr_many RPAREN stmt ELSE stmt"""
    p[0] = IFELSE(p[3], p[5], p[7], Ln(p.linespan(0)))


# CONTINUE
def p_stmt_8(p):
    """stmt : CONTINUE SEMICOL"""
    p[0] = CONTINUE(Ln(p.linespan(0)))


# BREAK
def p_stmt_9(p):
    """stmt : BREAK SEMICOL"""
    p[0] = BREAK(Ln(p.linespan(0)))


# RETURN VOID
def p_stmt_10(p):
    """stmt : RETURN SEMICOL"""
    p[0] = RETURN(Ln(p.linespan(0)))


# RETURN VALUE
def p_stmt_11(p):
    """stmt : RETURN expr_many SEMICOL"""
    p[0] = RETURN(Ln(p.linespan(0)), p[2])


# primary expression
def p_priexpr_1(p):
    """priexpr : ID"""
    p[0] = ID(p[1], Ln(p.linespan(0)))
...
def p_unaryexpr_2(p):
    ...
    p[0] = ADDR(p[2], Ln(p.linespan(0)))


def p_unaryexpr_3(p):
    """unaryexpr : TIMES unaryexpr"""
    p[0] = DEREF(p[2], Ln(p.linespan(0)))
...
def p_unaryexpr_4(p):
    """unaryexpr : INC unaryexpr
                 ...
                 | NOT unaryexpr
                 | LNOT unaryexpr"""
    p[0] = PREOP(p[1], p[2], Ln(p.linespan(0)))
...
def p_multexpr_2(p):
    ...
    p[0] = BINOP(p[1], p[2], p[3], Ln(p.linespan(0)))
...
def p_expr_2(p):
    ...
    if not (isinstance(p[1], ID) or (isinstance(p[1], SUBSCR) and isinstance(p[1].arrexpr, ID))):
        print("Syntax Error : line %d (lvalue required as left operand of assignment)" % p.lineno(1))
        exit(-1)
    if p[2] == "":
        p[0] = ASSIGN(p[1], p[3], Ln(p.linespan(0)))
    else:
        p[0] = ASSIGN(p[1], BINOP(p[1], p[2], p[3], Ln(p.linespan(0))), Ln(p.linespan(0)))


def p_expr_3(p):
    """expr : TIMES unaryexpr assign_op expr"""
    if p[3] == "":
        p[0] = ASSIGN(DEREF(p[2], Ln((p.lineno(1), p.lineno(2)))), p[4], Ln(p.linespan(0)))
    else:
        p[0] = ASSIGN(DEREF(p[2], Ln((p.lineno(1), p.lineno(2)))), 
                    BINOP(DEREF(p[2], Ln((p.lineno(1), p.lineno(2)))), p[3], p[4], Ln(p.linespan(0))),
                        Ln(p.linespan(0)))
...
def p_const_1(p):
    """const : IVAL"""
    p[0] = IVAL(p[1], Ln(p.linespan(0)))


def p_const_2(p):
    """const : FVAL"""
    p[0] = FVAL(p[1], Ln(p.linespan(0)))


def p_const_3(p):
    """const : SVAL"""
    p[0] = SVAL(p[1], Ln(p.linespan(0)))


def p_const_4(p):
    """const : CVAL"""
    p[0] = CVAL(p[1], Ln(p.linespan(0)))
...
def p_error(p):
    print("Syntax Error : line %d (YACC)" % p.lineno)
    exit(-1)
