...
def p_defv_1(p):
    ...
    p[0] = VDEF(p[1], p[2], Ln(p.lineno(1), p.lineno(3)))
...
def p_id_adv_1(p):
    """id_adv : ID"""
    p[0] = VDEFID(p[1], 0, None, Ln(p.lineno(1), p.lineno(1)))
...
def p_deff(p):
    ...
    p[0] = FDEF(p[1], p[2], p[4], p[7], Ln(p.lineno(1), p.lineno(8)))
...
def p_body_1(p):
    ...
    p[0] = BODY(p[1], p[2],Ln(p.lineno(1), p.lineno(2)))
...
def p_stmt_1(p):
    """stmt : SEMICOL"""
    p[0] = EMPTY_STMT(Ln(p.lineno(1), p.lineno(1)))
...
def p_stmt_4(p):
    ...
    p[0] = WHILE(p[3], p[5], Ln(p.lineno(1), p.lineno(5)))


# FOR
def p_stmt_5(p):
    """stmt : FOR LPAREN expr_many SEMICOL expr_many SEMICOL expr_many RPAREN stmt"""
    p[0] = FOR(p[3], p[5], p[7], p[9], Ln(p.lineno(1), p.lineno(9)))


# COND, IF
def p_stmt_6(p):
    """stmt : IF LPAREN expr_many RPAREN stmt"""
    p[0] = IFELSE(p[3], p[5], None, Ln(p.lineno(1), p.lineno(5)))


# COND, IF - ELSE
def p_stmt_7(p):
    """stmt : IF LPAREN expr_many RPAREN stmt ELSE stmt"""
    p[0] = IFELSE(p[3], p[5], p[7], Ln(p.lineno(1), p.lineno(7)))


# CONTINUE
def p_stmt_8(p):
    """stmt : CONTINUE SEMICOL"""
    p[0] = CONTINUE(Ln(p.lineno(1), p.lineno(2)))


# BREAK
def p_stmt_9(p):
    """stmt : BREAK SEMICOL"""
    p[0] = BREAK(Ln(p.lineno(1), p.lineno(2)))


# RETURN VOID
def p_stmt_10(p):
    """stmt : RETURN SEMICOL"""
    p[0] = RETURN(Ln(p.lineno(1), p.lineno(2)))


# RETURN VALUE
def p_stmt_11(p):
    """stmt : RETURN expr_many SEMICOL"""
    p[0] = RETURN(Ln(p.lineno(1), p.lineno(3)), p[2])


# primary expression
def p_priexpr_1(p):
    """priexpr : ID"""
    p[0] = ID(p[1], Ln(p.lineno(1), p.lineno(1))
...
def p_postexpr_2(p):
    ...
    p[0] = SUBSCR(p[1], p[3], Ln(p.lineno(1), p.lineno(4)))


# function call with void parameter
def p_postexpr_3(p):
    """postexpr : postexpr LPAREN RPAREN"""
    p[0] = CALL(p[1], [], Ln(p.lineno(1), p.lineno(3)))


# function call with parameters
def p_postexpr_4(p):
    """postexpr : postexpr LPAREN argexpr_list RPAREN"""
    p[0] = CALL(p[1], p[3], Ln(p.lineno(1), p.lineno(4)))


def p_postexpr_5(p):
    """postexpr : postexpr INC
                | postexpr DEC"""
    p[0] = POSTOP(p[1], p[2], Ln(p.lineno(1), p.lineno(2)))
...
def p_unaryexpr_2(p):
    ...
    p[0] = ADDR(p[2], Ln(p.lineno(1), p.lineno(2)))


def p_unaryexpr_3(p):
    """unaryexpr : TIMES unaryexpr"""
    p[0] = DEREF(p[2], Ln(p.lineno(1), p.lineno(2)))
...
def p_unaryexpr_4(p):
    """unaryexpr : INC unaryexpr
                 ...
                 | NOT unaryexpr
                 | LNOT unaryexpr"""
    p[0] = PREOP(p[1], p[2], Ln(p.lineno(1), p.lineno(2)))
...
def p_multexpr_2(p):
    ...
    p[0] = BINOP(p[1], p[2], p[3], Ln(p.lineno(1), p.lineno(3)))
...
def p_expr_2(p):
    ...
    if p[2] == "":
        p[0] = ASSIGN(p[1], p[3], Ln(p.lineno(1), p.lineno(3)))
    else:
        p[0] = ASSIGN(p[1], BINOP(p[1], p[2], p[3], Ln(p.lineno(1), p.lineno(3)), Ln(p.lineno(1), p.lineno(3)))


def p_expr_3(p):
    """expr : TIMES unaryexpr assign_op expr"""
    if p[3] == "":
        p[0] = ASSIGN(DEREF(p[2], Ln(p.lineno(1), p.lineno(2))), p[4], Ln(p.lineno(1), p.lineno(4)))
    else:
        p[0] = ASSIGN(DEREF(p[2], Ln(p.lineno(2), p.lineno(2))), 
                    BINOP(DEREF(p[2], Ln(p.lineno(1), p.lineno(2))), p[3], p[4], Ln(p.lineno(2), p.lineno(4))),
                        Ln(p.lineno(1), p.lineno(4)))
...
def p_const_1(p):
    """const : IVAL"""
    p[0] = IVAL(p[1], Ln(p.lineno(1), p.lineno(1)))


def p_const_2(p):
    """const : FVAL"""
    p[0] = FVAL(p[1], Ln(p.lineno(1), p.lineno(1)))


def p_const_3(p):
    """const : SVAL"""
    p[0] = SVAL(p[1], Ln(p.lineno(1), p.lineno(1)))


def p_const_4(p):
    """const : CVAL"""
    p[0] = CVAL(p[1], Ln(p.lineno(1), p.lineno(1)))
