...
def id2type(id, glob_var, func_arg, local_var):
    global STR_DATA
    if isinstance(id, int):
        return VDEFID("int", 0, None, Ln((-1, -1)))
    if isinstance(id, ID):
        id = id.name
    for var in local_var:
        if var[1] == id:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Compiler for C code')
    parser.add_argument('target', metavar='<file>', help='C file to compile')
    ...
    try:
        result = AST_ASM(AST_TYPE(AST_YACC(code)))
